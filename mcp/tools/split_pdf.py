...
def _json_obj_from_response(resp: httpx.Response) -> Any:
    ...
    if not body.startswith((b"{", b"[")):
        raise ValueError(
            f"Expected JSON response from SplitPDF, got content-type "
...
def _decode_split_item(item: dict[str, Any], index: int) -> tuple[bytes, str]:
    ...
    name = (
        ...
        or item.get("docName")
        or item.get("DocName")
        or f"split_{index + 1}.pdf"
    )
    if not isinstance(name, str) or not name.strip():
        name = f"split_{index + 1}.pdf"
    ...
    try:
        ...
        raise ValueError(f"Invalid base64 content for split item #{index + 1}: {exc}") from exc


async def _poll_split_pdf_job(
    ...
    *,
    ...
    raise TimeoutError(
        f"SplitPDF did not finish after {max_attempts} polls ({interval_sec}s apart)"
    )


async def _call_split_pdf_api(payload: dict[str, Any], pdf4me_api_key: str) -> Any:
    api_base_url = config.pdf4me_base_url.rstrip("/")
    url = f"{api_base_url}/api/v2/SplitPdf"
    headers = {
    ...
    async with httpx.AsyncClient(timeout=300) as client:
        ...
        if resp.status_code == 202:
            ...
            if not location:
                raise ValueError("API returned 202 Accepted but no Location header for polling")
            poll_url = resolve_polling_url(api_base_url, location)
            final = await _poll_split_pdf_job(
                client,
                poll_url,
                headers,
...
def _default_output_dir(pdf_file_path: str) -> str:
    ...
    return str(p.parent / f"split_pdf_{p.stem}")


@tool(
    name="split_pdf",
    description=(
        "Split a PDF via PDF4me SplitPDF (/api/v2/SplitPdf). "
        "split_action supports RecurringSplitAfterPage, SplitAfterPage, SplitSequence, SplitRanges. "
        "Provide split_action_number for first two, split_sequence for SplitSequence, split_ranges for SplitRanges."
    ),
)
async def split_pdf(
    pdf_file_path: str,
    split_action: SplitAction,
    split_action_number: Optional[int] = None,
    split_sequence: Optional[list[int]] = None,
    split_ranges: Optional[str] = None,
    ...
    if split_action in ("RecurringSplitAfterPage", "SplitAfterPage") and split_action_number is None:
        return ToolResult(
            content=f"split_action_number is required for split_action='{split_action}'."
        )
    if split_action == "SplitSequence" and not split_sequence:
        return ToolResult(content="split_sequence is required for split_action='SplitSequence'.")
    if split_action == "SplitRanges" and not split_ranges:
        return ToolResult(content="split_ranges is required for split_action='SplitRanges'.")
    ...
    if split_action_number is not None:
        payload["splitActionNumber"] = split_action_number
    if split_sequence is not None:
        payload["splitSequence"] = split_sequence
    if split_ranges is not None:
        payload["splitRanges"] = split_ranges

    resolved_out = output_dir if output_dir else _default_output_dir(pdf_file_path)
    os.makedirs(resolved_out, exist_ok=True)

    try:
        response_payload = await _call_split_pdf_api(payload, pdf4me_api_key)
    ...
    raw_json_path = os.path.join(resolved_out, "split_pdf_response.json")
    try:
    ...
    if not items:
        return ToolResult(
            content=(
                f"Split API response saved to {raw_json_path}, but no split documents were found "
                "in known fields (splitedDocuments/outputDocuments/list)."
            ),
            structured_content={"output_directory": resolved_out, "raw_json_path": raw_json_path},
        )
    ...
    for i, item in enumerate(items):
        try:
            ...
            return ToolResult(content=str(exc))
        try:
            output_paths.append(write_file_from_bytes(pdf_bytes, resolved_out, file_name))
        except OSError as exc:
            return ToolResult(content=f"Failed to write split output file: {exc}")

    return ToolResult(
        content=f"Split completed successfully. Saved {len(output_paths)} PDF file(s) to {resolved_out}",
        structured_content={
