...
BarcodeType = Literal["any", "datamatrix", "qrcode", "pdf417", "Code128"]
SplitBarcodePage = Literal["before", "after", "remove", "Before", "After", "Remove"]
PdfRenderDpi = Literal["100", "150", "200", "250", "300"]
...
def _json_obj_from_response(resp: httpx.Response) -> Any:
    ...
    if not body.startswith((b"{", b"[")):
        raise ValueError(
            f"Expected JSON response from SplitByBarcode, got content-type "
...
def _decode_split_item(item: dict[str, Any], index: int) -> tuple[bytes, str]:
    ...
    name = (
        ...
        or item.get("docName")
        or item.get("DocName")
        or f"split_by_barcode_{index + 1}.pdf"
    )
    if not isinstance(name, str) or not name.strip():
        name = f"split_by_barcode_{index + 1}.pdf"
...
async def _poll_split_pdf_by_barcode_job(
    ...
    *,
    ...
    raise TimeoutError(
        f"SplitByBarcode did not finish after {max_attempts} polls ({interval_sec}s apart)"
    )


async def _call_split_pdf_by_barcode_api(
    ...
    async with httpx.AsyncClient(timeout=300) as client:
        ...
        if resp.status_code == 202:
            ...
            final = await _poll_split_pdf_by_barcode_job(
                client,
                poll_url,
                headers,
...
def _default_output_dir(pdf_file_path: str) -> str:
    ...
    return str(p.parent / f"split_pdf_by_barcode_{p.stem}")


@tool(
    name="split_pdf_by_barcode",
    description=(
        "Split a PDF by barcode text via PDF4me SplitByBarcode (/api/v2/SplitByBarcode). "
        "Provide barcode_string, barcode_filter, barcode_type, split_barcode_page, "
        "combine_pages_with_same_consecutive_barcodes, and pdf_render_dpi."
    ),
)
async def split_pdf_by_barcode(
    pdf_file_path: str,
    barcode_string: str,
    barcode_filter: BarcodeFilter = "contains",
    barcode_type: BarcodeType = "any",
    split_barcode_page: SplitBarcodePage = "after",
    combine_pages_with_same_consecutive_barcodes: bool = False,
    pdf_render_dpi: PdfRenderDpi = "300",
    ...
    payload: dict[str, Any] = {
        ...
        "barcodeFilter": barcode_filter,
        "barcodeType": barcode_type,
        "splitBarcodePage": split_barcode_page,
        "combinePagesWithSameConsecutiveBarcodes": combine_pages_with_same_consecutive_barcodes,
    ...
    try:
        response_payload = await _call_split_pdf_by_barcode_api(payload, pdf4me_api_key)
    ...
    raw_json_path = os.path.join(resolved_out, "split_pdf_by_barcode_response.json")
    try:
    ...
    if not items:
        return ToolResult(
            content=(
                f"SplitByBarcode response saved to {raw_json_path}, but no split documents were found "
    ...
    return ToolResult(
        content=(
            f"Split by barcode completed successfully. Saved {len(output_paths)} PDF file(s) "
            f"to {resolved_out}"
        ),
        structured_content={
            ...
            "split_barcode_page": split_barcode_page,
