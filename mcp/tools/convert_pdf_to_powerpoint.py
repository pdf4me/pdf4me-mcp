...
def _pptx_filename_hint(doc_name: str) -> str:
    if doc_name.lower().endswith(".pdf"):
        return doc_name[:-4] + ".pptx"
    if doc_name.lower().endswith(".pptx"):
        return doc_name
    return f"{doc_name}.pptx"
...
def _bytes_from_pptx_response(resp: httpx.Response) -> bytes:
    """Binary PPTX from Content-Type or body; otherwise JSON with Document.DocData base64."""
    ct = (resp.headers.get("content-type") or "").lower()
    raw = resp.content

    if "presentationml" in ct or "application/octet-stream" in ct:
        return raw
    ...
    raise ValueError(
        f"Expected PowerPoint binary or JSON with DocData, got content-type {ct!r}"
    )


async def _call_convert_pdf_to_powerpoint_api(
    ...
    *,
    ...
    async with httpx.AsyncClient(timeout=120) as client:
        resp = await client.post(
            f"{api_base_url}/api/v2/ConvertPdfToPowerPoint",
            json=payload,
            headers=headers,
        )
        if resp.status_code == 202:
            ...
            return await _poll_convert_pdf_to_powerpoint_job(
                client,
                poll_url,
                headers,
        ...
        return _bytes_from_pptx_response(resp)


async def _poll_convert_pdf_to_powerpoint_job(
    ...
    *,
    ...
    for attempt in range(max_attempts):
        ...
        if poll.status_code == 200:
            return _bytes_from_pptx_response(poll)
        if poll.status_code == 202:
            continue
        poll.raise_for_status()
    raise TimeoutError(
        f"PDF to PowerPoint did not finish after {max_attempts} polls ({interval_sec}s apart)"
    )


@tool(
    name="convert_pdf_to_powerpoint",
    description=(
...
async def convert_pdf_to_powerpoint_http(
    file_path: str,
    ...
    """Convert PDF to PowerPoint via PDF4me ConvertPdfToPowerPoint.
    ...
    resolved_output_name = (
        output_file_name if output_file_name else _pptx_filename_hint(doc_name)
    )
    ...
    try:
        pptx_bytes = await _call_convert_pdf_to_powerpoint_api(
            doc_content_base64,
            doc_name,
            PDF4ME_API_KEY,
    ...
    if not pptx_bytes or not pptx_bytes.startswith(b"PK") or len(pptx_bytes) < 100:
        return ToolResult(
            content="Unexpected API response — PowerPoint file missing or invalid."
        )

    output_path = os.path.join(resolved_output_dir, resolved_output_name)
    try:
        write_file_from_bytes(
            pptx_bytes, resolved_output_dir, resolved_output_name)
    except OSError as exc:
        return ToolResult(content=f"Failed to write output file '{output_path}': {exc}")

    return ToolResult(
        content=f"PDF converted to PowerPoint successfully. Saved to {output_path}",
