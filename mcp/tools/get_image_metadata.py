...
async def _call_image_metadata_api(
    ...
    *,
    ...
    async with httpx.AsyncClient(timeout=120) as client:
        resp = await client.post(
            f"{api_base_url}/api/v2/GetImageMetadata",
            json=payload,
            headers=headers,
        )
        if resp.status_code == 202:
            ...
            return await _poll_image_metadata_job(
                client,
                poll_url,
                headers,
...
async def _poll_image_metadata_job(
    ...
    *,
    ...
    raise TimeoutError(
        f"GetImageMetadata did not finish after {max_attempts} polls ({interval_sec}s apart)"
    )
...
async def get_image_metadata_http(
    ...
    if extension.lower() not in allowed_extensions:
        return ToolResult(
            content=f"Input file must be JPG or PNG, got '{extension}' instead."
        )
    ...
    try:
        metadata = await _call_image_metadata_api(
            doc_content_base64,
            doc_name,
            PDF4ME_API_KEY,
    ...
    return ToolResult(
        content="Image metadata retrieved successfully.",
        structured_content={
            "file_name": doc_name,
            "metadata": metadata,
        },
