...
async def _poll_read_barcodes_job(
    ...
    *,
    ...
    for attempt in range(max_attempts):
        logger.debug(
            "ReadBarcodes poll: attempt %s/%s, sleeping %ss before GET",
            attempt + 1,
            max_attempts,
            interval_sec,
        )
        ...
        logger.info(
            "ReadBarcodes poll: attempt %s/%s, status=%s",
            attempt + 1,
            max_attempts,
            poll.status_code,
        )
        if poll.status_code == 200:
            logger.info("ReadBarcodes poll: completed (200)")
            return _barcodes_from_response(poll)
        if poll.status_code == 202:
            logger.info("ReadBarcodes poll: still processing (202)")
            continue
...
async def read_barcodes_http(
    ...
    Args:
        ...
        use_async: When True, send isAsync and poll the Location URL on 202 (10 × 10s, delay before each poll).
