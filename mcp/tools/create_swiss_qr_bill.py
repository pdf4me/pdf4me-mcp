...
async def create_swiss_qr_bill_http(
    ...
    cr_at, err = _coerce_enum(AddressTypeSK, cr_address_type, "cr_address_type", optional=False)
    if err:
        return err
    ...
    if err:
        return err
    lang, err = _coerce_enum(LanguageType, language_type, "language_type", optional=True)
    if err:
        return err
    rt, err = _coerce_enum(ReferenceType, reference_type, "reference_type", optional=True)
    if err:
        return err
    sep, err = _coerce_enum(SeperatorLine, seperator_line, "seperator_line", optional=True)
    if err:
        return err
    ud_at, err = _coerce_enum(AddressTypeSK, ud_address_type, "ud_address_type", optional=True)
    if err:
        return err
    ...
    if not resolved_output_dir:
        resolved_output_dir = os.path.dirname(os.path.abspath(pdf_file_path.strip()))
