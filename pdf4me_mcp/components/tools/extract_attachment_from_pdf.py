...
def _decode_attachment_item(item: Any, index: int) -> tuple[bytes, str]:
    if not isinstance(item, dict):
        raise ValueError(
            f"Unexpected outputDocuments entry type: {type(item)!r}")
...
async def extract_attachment_from_pdf(
    ...
    if _response_looks_json(final_resp):
        try:
            data: Any = json.loads(
                _strip_utf8_bom_and_leading_ws(
                    final_resp.content).decode("utf-8")
            )
    ...
    try:
        zip_path = write_file_from_bytes(
            raw, resolved_out, "extracted_attachments.zip")
    except OSError as exc:
