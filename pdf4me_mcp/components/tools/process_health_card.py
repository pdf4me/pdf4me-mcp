...
_DEFAULT_DOC_NAME = "health_card.jpeg"
...
def _json_dict_from_response(resp: httpx.Response) -> dict[str, Any]:
    ...
    if not body.startswith(b"{"):
        raise ValueError(
            f"Expected JSON object from ProcessHealthCard, got content-type "
    ...
    if not isinstance(parsed, dict):
        raise ValueError("Expected JSON object from ProcessHealthCard")
    return parsed


def _effective_health_card_dict(result: dict[str, Any]) -> dict[str, Any]:
    for key in (
        "processHealthCardModel",
        "ProcessHealthCardModel",
        "healthCardData",
        "HealthCardData",
    ):
...
def _slug_from_doc_name(doc_name: str) -> str:
    stem = Path(doc_name).stem or "health_card"
    slug = re.sub(r"[^\w\-]+", "_", stem, flags=re.UNICODE).strip("_")
    return slug or "health_card"


def _default_output_dir_for_file(card_file_path: str) -> str:
    p = Path(card_file_path).resolve()
    return str(p.parent / f"process_health_card_{p.stem}")


def _default_output_dir_for_doc_name(doc_name: str) -> str:
    return str(Path.cwd() / f"process_health_card_{_slug_from_doc_name(doc_name)}")


async def _poll_process_health_card_job(
    ...
    raise TimeoutError(
        f"ProcessHealthCard did not finish after {max_attempts} polls ({interval_sec}s apart)"
    )


async def _call_process_health_card_api(
    payload: dict[str, Any],
    pdf4me_api_key: str,
) -> dict[str, Any]:
    api_base_url = config.pdf4me_base_url.rstrip("/")
    url = f"{api_base_url}/api/v2/ProcessHealthCard"
    headers = {
    ...
    async with httpx.AsyncClient(timeout=300) as client:
        ...
        if resp.status_code == 202:
            ...
            final = await _poll_process_health_card_job(
                client,
                poll_url,
                headers,
...
@tool(
    name="process_health_card",
    title="AI-Process Health Card",
    description=(
        "AI-Process Health Card: extract structured data from a health card image/PDF via PDF4me "
        "POST /api/v2/ProcessHealthCard. Request JSON is only docContent, docName, and IsAsync (must be true); "
        "no customFieldKeys or CustomFieldKeys. "
        "Provide exactly one of: pdf_file_path (local .pdf/.png/.jpg/.jpeg as Base64) "
        "or doc_content (Base64, blob id, or URL—JPEG and other images are valid; not PDF-only). "
        ...
        f"{_DEFAULT_DOC_NAME!r}; names without an extension get .jpeg. "
        "202 + Location poll; saves process_health_card.json."
    ),
)
async def process_health_card(
    ...
    if not has_path:
        return ToolResult(
            content=(
                "Provide exactly one of pdf_file_path (local health card file) or "
    ...
    if not resolved_doc_name.lower().endswith(_suffixes):
        if "." not in resolved_doc_name.lower():
            resolved_doc_name = f"{resolved_doc_name}.jpeg"
    ...
    try:
        result = await _call_process_health_card_api(payload, pdf4me_api_key)
    ...
    json_path = os.path.join(resolved_out, "process_health_card.json")
    try:
    ...
    data = _effective_health_card_dict(result)
    ...
    summary = (
        f"ProcessHealthCard result saved to {json_path}. "
