...
def _barcode_payload(
    ...
    height_in_pt: Optional[str],
    width_in_pt: Optional[str],
    margin_x_in_pt: Optional[str],
    margin_y_in_pt: Optional[str],
    ...
    if height_in_pt is not None:
        payload["heightInPt"] = height_in_pt
    if width_in_pt is not None:
        payload["widthInPt"] = width_in_pt
    if margin_x_in_pt is not None:
        payload["marginXInPt"] = margin_x_in_pt
    if margin_y_in_pt is not None:
        payload["marginYInPt"] = margin_y_in_pt
    ...
    if is_text_above is not None:
        payload["isTextAbove"] = is_text_above
    return payload


async def _call_add_barcode_api(
    payload: dict,
    PDF4ME_API_KEY: str,
) -> bytes:
    api_base_url = config.pdf4me_base_url.rstrip("/")
    url = f"{api_base_url}/api/v2/addbarcode"
    headers = {
    ...
    async with httpx.AsyncClient(timeout=120) as client:
        ...
        if resp.status_code == 202:
            ...
            return await _poll_add_barcode_job(
                client,
                poll_url,
                headers,
...
async def _poll_add_barcode_job(
    ...
    *,
    ...
    raise TimeoutError(
        f"AddBarcode did not finish after {max_attempts} polls ({interval_sec}s apart)"
    )
...
@tool(
    ...
    description=(
        "Draw a barcode or QR code onto PDF pages using the PDF4me addbarcode API (/api/v2/addbarcode). "
        "Requires local pdf_file_path, barcode text, barcodeType (e.g. qrCode, code128), pages spec "
        "(e.g. all, 1, 1-3, 1,3,5), alignX (Left/Center/Right), alignY (Top/Middle/Bottom), hideText. "
        "Optional: size/margins in MM or points, opacity, displayText, showOnlyInPrint, isTextAbove."
    ),
)
async def add_barcode_to_pdf(
    pdf_file_path: str,
    text: str,
    ...
    height_in_pt: Optional[str] = None,
    width_in_pt: Optional[str] = None,
    margin_x_in_pt: Optional[str] = None,
    margin_y_in_pt: Optional[str] = None,
    opacity: Optional[int] = None,
    display_text: Optional[Literal["above", "below"]] = None,
    show_only_in_print: Optional[bool] = None,
    is_text_above: Optional[bool] = None,
    ...
    payload = _barcode_payload(
        ...
        height_in_pt=height_in_pt,
        width_in_pt=width_in_pt,
        margin_x_in_pt=margin_x_in_pt,
        margin_y_in_pt=margin_y_in_pt,
        opacity=opacity,
    ...
    try:
        pdf_bytes = await _call_add_barcode_api(payload, PDF4ME_API_KEY)
    ...
    return ToolResult(
        content=f"PDF with barcode saved successfully to {output_path}",
        structured_content={
