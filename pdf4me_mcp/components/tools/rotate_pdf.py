...
async def _call_rotate_pdf_api(
    ...
    *,
    ...
    async with httpx.AsyncClient(timeout=120) as client:
        ...
        if resp.status_code == 202:
            ...
            return await _poll_rotate_pdf_job(
                client,
                poll_url,
                headers,
...
async def _poll_rotate_pdf_job(
    ...
    *,
    ...
    raise TimeoutError(
        f"Rotate PDF did not finish after {max_attempts} polls ({interval_sec}s apart)"
    )
...
async def rotate_pdf_http(
    ...
    """Rotate every page of a PDF using the PDF4me Rotate API.
    ...
    resolved_output_name = output_file_name if output_file_name else f"rotated_{doc_name}"
    ...
    try:
        pdf_bytes = await _call_rotate_pdf_api(
            doc_content_base64,
    ...
    return ToolResult(
        content=f"PDF rotated successfully. Saved to {output_path}",
