...
def _json_dict_from_response(resp: httpx.Response) -> dict[str, Any]:
    ...
    if not body.startswith(b"{"):
        raise ValueError(
            f"Expected JSON object from ProcessContract, got content-type "
    ...
    if not isinstance(parsed, dict):
        raise ValueError("Expected JSON object from ProcessContract")
    return parsed


def _effective_contract_dict(result: dict[str, Any]) -> dict[str, Any]:
    for key in (
...
async def _poll_process_contract_job(
    ...
    raise TimeoutError(
        f"ProcessContract did not finish after {max_attempts} polls ({interval_sec}s apart)"
    )


async def _call_process_contract_api(
    payload: dict[str, Any],
    pdf4me_api_key: str,
) -> dict[str, Any]:
    api_base_url = config.pdf4me_base_url.rstrip("/")
    url = f"{api_base_url}/api/v2/ProcessContract"
    headers = {
    ...
    async with httpx.AsyncClient(timeout=300) as client:
        ...
        if resp.status_code == 202:
            ...
            final = await _poll_process_contract_job(
                client,
                poll_url,
                headers,
...
async def process_contract(
    ...
    try:
        result = await _call_process_contract_api(payload, pdf4me_api_key)
    ...
    json_path = os.path.join(resolved_out, "process_contract.json")
    try:
    ...
    data = _effective_contract_dict(result)
    ...
    summary = (
        f"ProcessContract result saved to {json_path}. "
