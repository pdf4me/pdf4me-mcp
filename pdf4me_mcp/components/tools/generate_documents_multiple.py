...
_TemplateFileType = Literal["Docx", "Html",
                            "Pdf", "MailMerge", "GoogleDocs", "Word"]
...
def _bytes_list_from_multiple_json(payload: Any) -> list[bytes]:
    if not isinstance(payload, dict):
        raise ValueError(
            "Expected JSON object response for GenerateDocumentMultiple")
    docs = payload.get("outputDocuments")
    if not isinstance(docs, list) or not docs:
        raise ValueError(
            "Response JSON has no non-empty outputDocuments array")
    out: list[bytes] = []
    for doc in docs:
...
async def generate_documents_multiple_http(
    ...
    try:
        if len(output_parts) == 1:
            write_file_from_bytes(
                output_parts[0], resolved_output_dir, base_name)
    ...
    return ToolResult(
        ...
        structured_content={"output_paths": saved_paths,
