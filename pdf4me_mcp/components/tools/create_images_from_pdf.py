...
from pdf4me_mcp.helper import file_to_base64, resolve_polling_url, write_file_from_base64
...
def _parse_images_response(resp: httpx.Response) -> list[tuple[str, str]]:
    ...
    if "application/json" not in ct and "json" not in ct:
        raise ValueError(
            f"Expected JSON response for CreateImages, got content-type {ct!r}")
    try:
        data = resp.json()
    except json.JSONDecodeError as exc:
        raise ValueError(
            f"Invalid JSON in CreateImages response: {exc}") from exc
...
async def create_images_from_pdf_http(
    ...
    try:
        for b64, name in images:
            saved.append(write_file_from_base64(
                b64, resolved_output_dir, name))
    ...
    return ToolResult(
        ...
        structured_content={"output_paths": saved,
