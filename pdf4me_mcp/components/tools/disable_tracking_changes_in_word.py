...
async def _poll_disable_tracking_job(
    ...
    *,
    ...
    raise TimeoutError(
        "DisableTrackingChangesInWord did not finish after "
        f"{max_attempts} polls ({interval_sec}s apart)"
    )


async def _call_disable_tracking_api(
    doc_name: str,
    doc_content_base64: str,
    pdf4me_api_key: str,
) -> bytes:
    api_base_url = config.pdf4me_base_url.rstrip("/")
    url = f"{api_base_url}/api/v2/DisableTrackingChangesInWord"
    payload = {
        "docName": doc_name,
    ...
    async with httpx.AsyncClient(timeout=180) as client:
        ...
        if resp.status_code == 202:
            ...
            return await _poll_disable_tracking_job(
                client,
                poll_url,
                headers,
...
@tool(
    name="disable_tracking_changes_in_word",
    title="Disable Track Changes in Word",
    description=(
        "Disable Word Track Changes via PDF4me /api/v2/DisableTrackingChangesInWord. "
        "Input is a local Word file path (.docx or .doc) which is sent as docContent Base64; "
        "docName defaults to the input filename. Saves a Word output file with track changes disabled."
    ),
)
async def disable_tracking_changes_in_word(
    word_file_path: str,
    ...
    resolved_output_name = (
        output_file_name if output_file_name else f"tracking_disabled_{doc_name}"
    )

    try:
        word_bytes = await _call_disable_tracking_api(doc_name, doc_b64, pdf4me_api_key)
    ...
    return ToolResult(
        content=f"Track changes disabled successfully. Saved to {output_path}",
