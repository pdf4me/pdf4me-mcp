...
@tool(
    ...
    description=(
        ...
        "docName resolution: uses doc_name if set else the file basename. "
        ...
        "or square bracket. "
        "202 + Location poll; saves process_shipping_label.json."
    ),
)
async def process_shipping_label(
    ...
    if not has_path:
        return ToolResult(
            content="Provide pdf_file_path (local shipping label file)."
        )
